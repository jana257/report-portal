  ...
  const chartData = useMemo(
    () =>
      rows.slice(0, 10).map((r) => ({
        id: r.venueId,
        name: r.venueName?.trim() || "Nepoznata lokacija",
        city: r.venueCity?.trim() || "Nepoznat grad",
        fullLabel: `${r.venueName?.trim() || "Nepoznata lokacija"} • ${
          r.venueCity?.trim() || "Nepoznat grad"
        }`,
        value: r.ticketsSold,
      })),
    [rows]
          ...
          <table className="table">
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <tr key={r.venueId}>
                    <td>{r.venueName?.trim() || "Nepoznata lokacija"}</td>
                    <td>{r.venueCity?.trim() || "Nepoznat grad"}</td>
